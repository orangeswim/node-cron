(function(root, factory) {
	...
	CronTime.prototype = {
		 ...
		 */
		sendAt: function(i, startDate) {
			var date = this.realDate ? this.source : moment(startDate);
		...
		/**
		 * calculates the next send time until the date
		 */
		sendUntil: function(endTime, startTime) {
			if(endTime == undefined){
				throw new Error('WARNING: No endTime given.');
			}

			if (endTime instanceof Date || endTime._isAMomentObject) {
			...
			var date;
			if (startTime && (startTime instanceof Date || startTime._isAMomentObject)) {
				date = moment(startTime);
			}else{
				date = this.realDate ? this.source : moment();
			}
			...
			var diff = date.diff(endTime,'s');
			if( diff > 0){
				throw new Error('WARNING: Date before source date. Will never be fired.');
			}
	...
	CronJob.prototype.nextDates = function(i,startDate) {
		return this.cronTime.sendAt(i,startDate);
	};

	CronJob.prototype.datesUntil = function(endTime, startTime) {
		return this.cronTime.sendUntil(endTime, startTime);
	};
